...
class GeminiWebSourceBackend:
    ...
    async def search(self, instruction: MonitoringInstruction) -> list[SourceFinding]:
        ...
        obligations = _obligations_from_grounded_text(
            text=text,
        ...
        return [
            SourceFinding(
                summary=text.strip() or f"Grounded search completed for {instruction.query}.",
                obligations=obligations,
                evidence=evidence,
                citations=citations,
                confidence=0.66 if evidence else 0.35,
                uncertainty=(
...
def _obligations_from_grounded_text(
    text: str,
    ...
    obligations = [
        ...
        for obligation_id, obligation_text, topics in candidates
        if any(topic.lower() in lower_text for topic in topics)
    ]
    if obligations:
        return obligations
    return [
        RegulatoryObligation(
            id="obl-grounded-summary",
            text=f"Review internal controls for regulatory update: {instruction.query}",
            jurisdiction=jurisdiction,
            topics=[domain],
            risk_level="medium",
            source_url=source_url,
            source_quote=text[:500] or instruction.query,
            confidence="low",
        )
    ]
